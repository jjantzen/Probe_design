...
- average total sequence length across samples (per locus) (with and without zero lengths) = 1 value per locus per clade
- minimum and maximum sequence length across samples (per locus) = 2 values per locus per clade
- pop level: aggregate values across species not across samples

``` {r sequence_length_all_calcs}

#read file - output from sequencing success stats file

#for combo sets
#seq_lengths <- read.csv("../Output_files/Combo_files/JJ_combo_lengths.csv", row.names = 1, stringsAsFactors = FALSE)

#for all JJ run at once
seq_lengths_JJ <- read.csv("../Output_files/All_files/JJ_all_lengths.csv", row.names = 1, stringsAsFactors = FALSE)
...
seq_lengths_PA <- read.csv("../Output_files/All_files/PA_all_lengths.csv", row.names = 1, stringsAsFactors = FALSE)
seq_lengths_PA <- read.csv("../Output_files/paper_scripts_output/PA_all_lengths.csv", row.names = 1, stringsAsFactors = FALSE)

colnames(seq_lengths_JJ)
# #check for worst samples
# seq_lengths_PA %>% 
#   group_by(sample) %>% 
...
#seq_lengths_PA_2 <- seq_lengths_PA[which(seq_lengths_PA$Var1 != "501_Msp_clean"),]
...
# head(seq_lengths)
# seq_lengths <- seq_lengths_JJ

#write seq length function

seq_lengths <- seq_lengths_PA
...
``` {r stats_all_probes_one_function}
...
#need to get right format of changing species names (Prabha list)
...
#seq_stats_PA_2 <- seq_stats_PA[which(rownames(seq_stats_PA) != "501_Msp_clean"),]
...
names(stats_calc_output_JJ)

names(seq_lengths_calc_output_table)

# stats_calc_output <- stats_calc_output_PA
# seq_lengths_calc_output_table <- seq_lengths_calc_output_table_PA
...
bad <- JJ_out$locus_based$locus[-which(JJ_out$locus_based$locus %in% PA_out$locus_based$locus)]

PA_out$species_based

#need to verify that results are what we expected

```


``` {r group by method and genome1}
...
Sixth, sequence similarity between probes and (super)contigs:
- average sequence similarity = 1 value per clade
- minimum and maximum sequence similarity = 2 values per clade
- identify outliers (samples)
- pop level: aggregate values across species not across samples

Last:
- divide length into exon, intron and supercontig lengths = 3 values per locus per sample
- average across samples for exon, intron and supercontig lengths = 3 values per locus
- % of supercontig length represented by introns = 1 value per locus
- pop level: aggregate values across species not across samples??
