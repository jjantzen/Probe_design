...
# Just multiply the mean sequence length by the number of sequences.
...
colnames(length_data_JJ)
colnames(both_together_lengths)

head(both_together_lengths)
...
# #get avg read length
# 
# reduced_lengths <- both_together_lengths[which(both_together_lengths$Species != "MeanLength"),]
# nrow(reduced_lengths)
# 
# avg_lengths <- data.frame(sample = reduced_lengths$Species, avg_length = rowMeans(reduced_lengths[,-1]))
# 
# nrow(read_num)
# nrow(avg_lengths)
...
#or could do average
#c_tar_length <- (mean seq length x num seqs)
